...
case 'settle_info':
	...
	if(!empty($rows['usdt_chain'])) $data .= '<div class="form-group"><div class="input-group"><div class="input-group-addon">交易链/交易所</div><input type="text" value="'.$rows['usdt_chain'].'" class="form-control" disabled/></div></div>';
...
break;
case 'paypwd_check':
	if(isset($_SESSION['paypwd_verified']) && $_SESSION['paypwd_verified'] == ($adminInfo['id'] ?? 0))
		exit('{"code":0,"msg":"ok"}');
	else
...
case 'paypwd_input':
	...
	if(\lib\AdminAuth::verifyPayPassword($adminInfo['id'], $paypwd)){
		$_SESSION['paypwd_verified'] = $adminInfo['id'];
		exit('{"code":0,"msg":"ok"}');
	}else{
...
break;
case 'paypwd_reset':
	unset($_SESSION['paypwd_verified']);
	exit('{"code":0,"msg":"ok"}');
break;
...
case 'transfer':
	...
	if(!isset($_SESSION['paypwd_verified']) || $_SESSION['paypwd_verified'] != ($adminInfo['id'] ?? 0))exit('{"code":-1,"msg":"支付密码错误，请返回重新进入该页面"}');
