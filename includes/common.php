...
if (!function_exists("is_https")) {
    function is_https()
    {
        if (isset($_SERVER["SERVER_PORT"]) && $_SERVER["SERVER_PORT"] == 443) {
            ...
            isset($_SERVER["HTTP_X_CLIENT_SCHEME"]) &&
            ...
            isset($_SERVER["HTTP_EWS_CUSTOME_SCHEME"]) &&
            $_SERVER["HTTP_EWS_CUSTOME_SCHEME"] == "https"
        ) {
            return true;
        }
        return false;
...
if ($DB->query("SELECT 1 FROM pre_config LIMIT 1") === false) {
    //检测安装2
    header("Content-type:text/html;charset=utf-8");
    echo '你还没安装！<a href="/install/">点此安装</a>';
    exit();
...
if (
    !file_exists(ROOT . "install/install.lock") &&
    file_exists(ROOT . "install/index.php")
) {
    sysmsg(
        '<h2>检测到无 install.lock 文件</h2><ul><li><font size="4">如果您尚未安装本程序，请<a href="/install/">前往安装</a></font></li><li><font size="4">如果您已经安装本程序，请手动放置一个空的 install.lock 文件到 /install 文件夹下，<b>为了您站点安全，在您完成它之前我们不会工作。</b></font></li></ul><br/><h4>为什么必须建立 install.lock 文件？</h4>它是安装保护文件，如果检测不到它，就会认为站点还没安装，此时任何人都可以安装/重装你的网站。<br/><br/>',
    );
    exit();
}
